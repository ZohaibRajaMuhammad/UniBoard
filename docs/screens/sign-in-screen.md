...
## 1. Screen Overview
- Screen purpose: Authenticate existing users into their academic workspace.
- Business goal: Recover returning traffic efficiently and minimize login abandonment.
- User goal: Enter credentials, recover access quickly, and reach the dashboard with confidence.
- Primary actions:
...
- Return to landing
- Reveal password
- Persist session on trusted device

## 2. UI Layout Architecture
- Structural layout:
- Centered auth card, max width 420px to 440px
- Back button above or adjacent to card
- Stacked form fields with 12px to 16px gaps
- Primary submit CTA anchored below inputs with support links beneath
...
|---|---|---|---|---|---|---|---|---|---|---|
| signin-back | Back button | Ghost button | Desktop, tablet, mobile | Return to landing | Navigate | default, hover, focus | Public | Route state | None | Should preserve auth form if returning to sign-in from browser back |
| signin-email | Email field | Input | Desktop, tablet, mobile | Collect user identifier | Type | empty, focus, filled, invalid, disabled | Public | auth.email | Required, normalized email | May be prefilled from remembered session or invitation |
| signin-password | Password field | Input | Desktop, tablet, mobile | Collect secret credential | Type | empty, focus, filled, invalid, disabled | Public | auth.password | Required | Support reveal toggle in production |
| signin-forgot | Forgot password link | Text link | Desktop, tablet, mobile | Start recovery flow | Open recovery | default, hover, focus | Public | Recovery route | None | Should not leak whether email exists until after submission |
| signin-submit | Sign in submit | Primary button | Desktop, tablet, mobile | Authenticate user | Submit form | default, hover, focus, loading, disabled | Public | POST /auth/login | Enabled only when required fields present | Enter key submits |
| signin-switch | Sign-up switch link | Text link | Desktop, tablet, mobile | Move to registration | Navigate | default, hover, focus | Public | Route state | None | Keep below submit to support auth branching |
...
## 5. State Management
- Initial state: Initial empty form, validating, submitting, success redirect, inline error, locked account, offline retry.
...
## 6. Business Logic
- Screen logic:
- Institutional email may be required depending on tenant policy
- Failed attempts should trigger risk scoring and optional step-up auth
- Successful auth fetches profile, room roster, notification counts, and settings bootstrap
...
- SSO-enabled tenant hiding password login
- Multiple failed attempts leading to captcha or rate limit
