...
## 1. Screen Overview
- Screen purpose: Expose user-level preferences for notifications, AI features, and account behavior.
- Business goal: Increase controllability, trust, and retention while reducing support load.
- User goal: Tune alerts, privacy, and AI assistance to personal preference.
- Primary actions:
- Toggle settings
- Save changes
...
- Structural layout:
- Vertical settings groups with label left and toggle or control right
- AI-specific setting block highlighted in the refined prototype
- Section headers with compact helper copy
...
|---|---|---|---|---|---|---|---|---|---|---|
| settings-groups | Settings sections | Grouped rows | Desktop, tablet, mobile | Organize preferences | Read | default | Authenticated | settings groups | None | Use clear category labels |
| settings-toggle | Boolean toggles | Toggle switch | Desktop, tablet, mobile | Enable/disable preference | Tap/click | on, off, disabled, saving | Authenticated unless policy-locked | settings[key] | Allowed key and value | Must show lock reason when disabled |
| settings-ai-block | AI features section | Highlighted settings card | Desktop, tablet, mobile | Manage AI defaults | Toggle | visible, hidden by policy | Authenticated | ai settings | Tenant AI policy | Refined prototype emphasizes this section |
...
## 5. State Management
- Initial state: Initial state from bootstrap, dirty state, saving, saved, save failure, policy-locked.
...
## 6. Business Logic
- Screen logic:
- Some settings are tenant-enforced and read-only
- AI features can be globally disabled even if UI surfaces remain visible
- Changes should write through immediately or batch-save consistently
...
## 8. Edge Cases
- Partial save where one preference fails
- Offline toggle changes queued locally
- Conflict with change from another device
