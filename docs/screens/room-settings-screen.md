...
## 1. Screen Overview
- Screen purpose: Allow instructors or moderators to manage room metadata and operational rules.
- Business goal: Enable governed collaboration without separate admin tooling.
- User goal: Inspect room metadata, change room policies, and save administrative updates.
- Primary actions:
- Review room details
- Toggle room options
- Save changes
- Secondary actions:
- Inspect membership or visibility fields
- Back to room detail
- Create or invite members in future versions

## 2. UI Layout Architecture
- Structural layout:
- Two-column details/settings layout in refined prototype
- Metadata card left, policy toggles right
- Persistent save action at end of settings card
...
|---|---|---|---|---|---|---|---|---|---|---|
| room-settings-meta | Room details card | Read-only metadata list | Desktop, tablet, mobile | Show current room configuration | Passive | loading, loaded | Admin/member read scope | room metadata | None | May be member-visible even if edit panel is not |
| room-settings-toggles | Policy toggle rows | Toggle list | Desktop, tablet, mobile | Edit room rules | Toggle | on, off, disabled, saving | Owner/moderator | roomSettings | Role and tenant policy | Anonymous posting and AI enablement have downstream effects |
| room-settings-save | Save changes button | Primary button | Desktop, tablet, mobile | Persist policy edits | Click | default, dirty, loading, success, error | Owner/moderator | saveStatus | Only enabled when dirty and valid | Should warn on unsaved changes |
...
## 5. State Management
- Initial state: Loading room config, editing, save pending, save success, permission denied.
...
## 6. Business Logic
- Screen logic:
- Only room owners and designated admins can modify policy toggles
- Changes to anonymity and invites affect composer and join flows immediately
- AI room summary enablement toggles downstream AI jobs
...
## 8. Edge Cases
- Room archived while settings open
- Moderator lacks permission for visibility change
- Save succeeds partially when one policy conflicts
