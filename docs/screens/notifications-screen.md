...
## 1. Screen Overview
- Screen purpose: Aggregate alerts for replies, upvotes, pinned content, deadlines, and instructor actions.
- Business goal: Re-engage users with actionable alerts without overwhelming them.
- User goal: Review what changed, understand urgency, and jump directly to the related content.
- Primary actions:
- Read notifications
...
- Structural layout:
- Single-column list optimized for scanning
- Notification icon, body, timestamp, and unread marker aligned consistently
- Filter controls in the upper right or header row
...
|---|---|---|---|---|---|---|---|---|---|---|
| notif-filter | Unread/all filter | Tabs or pills | Desktop, tablet, mobile | Switch list subset | Click | default, active | Authenticated | activeFilter | Valid enum only | Unread is common default |
| notif-list | Notification list | List | Desktop, tablet, mobile | Display actionable alerts | Open item | loading, loaded, empty | Authenticated | notifications[] | Destination access must be rechecked | Unread visual treatment required |
| notif-row | Notification row | Clickable row | Desktop, tablet, mobile | Deep-link to source content | Click, mark read | unread, read, hover | Authenticated | notification item | If source missing, route to fallback | Can mark read on open |
...
## 5. State Management
- Initial state: Initial load, unread-filtered, all-filtered, pagination, mark-read optimistic state, fetch error.
...
## 6. Business Logic
- Screen logic:
- Unread count propagates to badges across sidebar and mobile nav
- High-urgency deadline alerts outrank social notifications
- Duplicate notifications from the same event should collapse
...
## 8. Edge Cases
- Destination content deleted
- Notification arrives after page already open
- User opts out of a class of notifications
