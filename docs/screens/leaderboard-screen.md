...
## 1. Screen Overview
- Screen purpose: Rank users by points, contributions, or engagement within rooms or across the workspace.
- Business goal: Increase healthy competition and reward quality participation.
- User goal: See current standing, understand how to improve, and compare against peers.
- Primary actions:
- Review rank list
- Inspect own position
- Switch scope or timeframe
- Secondary actions:
- Navigate to profile or reputation details
- Understand point deltas
- Review podium highlights

## 2. UI Layout Architecture
- Structural layout:
- Podium or highlighted top ranks above a vertically stacked list
- Monospace rank labels, avatar blocks, and delta indicators for momentum
...
|---|---|---|---|---|---|---|---|---|---|---|
| leaderboard-podium | Top ranks podium | Highlighted metric block | Desktop, tablet, mobile | Show top performers | Passive | loading, loaded | Authenticated | leaderboard.top | Rank ties deterministic | AI prototype features podium styling |
| leaderboard-list | Leaderboard rows | List | Desktop, tablet, mobile | Show rankings | Scroll, inspect | loading, loaded, empty | Authenticated | leaderboard.rows | Score and rank integrity | Own row should be distinct |
| leaderboard-filter | Scope filter | Select | Desktop, tablet, mobile | Change room or timeframe | Select | default, open | Authenticated | leaderboardScope | Valid scope enum | More explicit in reputation prototype |
...
## 5. State Management
- Initial state: Loading ranks, loaded, filter changed, stale ranking, empty cohort, fetch error.
...
## 6. Business Logic
- Screen logic:
- Ranking formula must align with reputation scoring definitions
- Ties require deterministic ordering
- Own rank should be anchored even if outside top visible slice
...
## 8. Edge Cases
- User removed from room but cached in board
- Score recalculation job in progress
- Abuse or moderation reversal affecting scores
