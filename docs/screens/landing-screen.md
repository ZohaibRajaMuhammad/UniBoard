...
## 1. Screen Overview
- Screen purpose: Market the academic collaboration platform, establish trust, and drive conversion into sign in or sign up.
- Business goal: Acquire new users, communicate product differentiation, and funnel prospects into account creation.
- User goal: Understand value quickly, evaluate relevance to academic workflows, and start onboarding.
- Primary actions:
- Open sign-up flow
- Open sign-in flow
- Scan product value propositions
- Secondary actions:
- Review hero messaging
- Inspect feature highlights
- Return to brand home state

## 2. UI Layout Architecture
- Structural layout:
- Top marketing header with logo left and auth CTAs right
- Hero block with headline, supporting copy, and primary CTA
- Feature/value strip describing rooms, deadlines, and AI assistance
- Responsive max-width container around 1280px to 1320px
...
|---|---|---|---|---|---|---|---|---|---|---|
| land-logo | Brand logo | Clickable brand mark | Desktop, tablet, mobile | Return user to marketing home state | Click | default, hover, focus | Public | Static asset | None | Should preserve campaign params if required |
| land-signin | Sign in CTA | Button | Desktop, tablet, mobile | Open sign-in screen | Navigate to sign-in | default, hover, focus, active | Public | Route state | None | Secondary emphasis |
| land-signup | Get started CTA | Button | Desktop, tablet, mobile | Open sign-up screen | Navigate to sign-up | default, hover, focus, active | Public | Route state | None | Primary conversion action |
| land-hero-cta | Hero primary CTA | Button | Desktop, tablet, mobile | Drive onboarding conversion | Navigate to sign-up | default, hover, focus | Public | Route state | None | Must stay above the fold on common laptop sizes |
| land-feature-cards | Feature/value cards | Informational cards | Desktop, tablet, mobile | Explain room, deadline, and AI value | Passive scan | default | Public | Static marketing content | None | Can later support experiment variants |
...
## 5. State Management
- Initial state: Static content by default; marketing analytics load asynchronously; CTA clicks transition immediately to auth screens.
...
## 6. Business Logic
- Screen logic:
- Unauthenticated-only route
- If user already has a valid session, redirect to dashboard
- Feature merchandising can be audience-personalized by institution or campaign
...
## 8. Edge Cases
- Stale active session token on first load
- Blocked fonts or slow network degrading layout
- Users opening sign-up directly from campaign links
