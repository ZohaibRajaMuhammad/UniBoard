...
## 1. Screen Overview
- Screen purpose: Explain the userâ€™s academic reputation, XP progression, expertise, achievements, and competitive standing.
- Business goal: Reinforce quality contribution loops and long-term retention.
- User goal: Understand current level, see progress to next tier, and identify the actions that increase XP.
- Primary actions:
- Review XP and tier progress
- Inspect subject expertise
- Read recent XP events
- Compare against leaderboard
- Secondary actions:
- Review streak
- Inspect unlocked achievements
- Change leaderboard scope

## 2. UI Layout Architecture
- Structural layout:
- Two-column layout with profile and achievements on the left, streak and leaderboard on the right
- Hero card with XP, tier chips, and progress track
- Dense but highly scannable metric cards and achievement grid
...
|---|---|---|---|---|---|---|---|---|---|---|
| rep-hero | Reputation hero card | Composite card | Desktop, tablet, mobile | Summarize identity, XP, tier, and progress | Passive | loading, loaded | Authenticated | reputation profile | XP total and tier consistency | Most information-dense card in the prototype |
| rep-xp-progress | XP progress track | Progress bar | Desktop, tablet, mobile | Show progress to next tier | Passive | loaded | Authenticated | xp current/target | 0-100 percent derived | Must align with tier thresholds |
| rep-expertise | Subject expertise cards | Card grid | Desktop, tablet, mobile | Show AI-inferred strengths | Review | loading, loaded, low-confidence | Authenticated | expertise[] | Confidence threshold and explainability | AI-derived, not primary scoring |
| rep-achievements | Achievement grid | Badge grid | Desktop, tablet, mobile | Show unlocked and locked milestones | Inspect | locked, unlocked | Authenticated | achievements[] | Threshold integrity | Need accessible text alternatives for icons |
| rep-activity | Recent XP activity feed | Timeline | Desktop, tablet, mobile | Explain recent score changes | Read | loading, loaded | Authenticated | xpEvents[] | Chronological ordering | Key transparency feature |
| rep-streak | Streak card | Metric visualization | Desktop, tablet, mobile | Show habit continuity | Passive | loaded | Authenticated | streak data | Day sequence consistency | Should honor timezone and local day boundary |
| rep-leaderboard-filter | Leaderboard scope select | Select | Desktop, tablet, mobile | Change ranking context | Select | default, open | Authenticated | leaderboardScope | Allowed scopes only | Appears in right rail |
...
## 5. State Management
- Initial state: Loading, loaded, leaderboard filter change, recalculation pending, moderation-adjusted score state.
...
## 6. Business Logic
- Screen logic:
- XP sources include upvotes, resolved answers, pinned posts, quality notes, and streaks
- AI-inferred expertise should not directly change scores without transparent explanation
- Abuse moderation must be able to reverse XP safely
...
## 8. Edge Cases
- Score recalculated retroactively
- Achievement revoked
- Leaderboard scope includes archived room
- AI expertise inference drifts from actual performance
