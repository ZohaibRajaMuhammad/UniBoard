...
## 1. Screen Overview
- Screen purpose: Collect user-saved posts, resources, and study artifacts for later retrieval.
- Business goal: Increase content reuse and long-tail value of academic contributions.
- User goal: Return to previously saved material without searching again.
- Primary actions:
- Browse saved items
- Open saved content
- Unsave item
...
- Filter by type
- Sort by recency
- Export or share in future versions

## 2. UI Layout Architecture
- Structural layout:
- Single-column saved list with compact cards
- Prominent empty state when there are no saved artifacts
...
|---|---|---|---|---|---|---|---|---|---|---|
| saved-list | Saved items list | List | Desktop, tablet, mobile | Display bookmarks | Open, unsave | loading, loaded, empty | Authenticated | saved[] | Source access recheck | Empty state should educate user on saving behavior |
| saved-row | Saved item card | Clickable card | Desktop, tablet, mobile | Return to content | Open or unsave | default, hover | Authenticated | saved item | Handle deleted content | Can include room and type metadata |
...
## 5. State Management
- Initial state: Initial load, populated list, empty state, unsave optimistic state, sync failure.
...
## 6. Business Logic
- Screen logic:
- Saved items preserve source references even when content location changes
- Deleted source content should show tombstone state
- Personal saved state is private by default
...
- GET /api/v1/saved
- POST /api/v1/saved/{entityType}/{entityId}
- DELETE /api/v1/saved/{entityType}/{entityId}
