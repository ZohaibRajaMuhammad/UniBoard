...
## 1. Screen Overview
- Screen purpose: Display user identity, summary metrics, and editable personal information.
- Business goal: Support account trust, personalization, and downstream analytics.
- User goal: Review profile details, inspect progress, and update identity attributes.
- Primary actions:
- View avatar and identity
- Edit profile fields
- Review personal stats
- Secondary actions:
- Inspect learning profile or badges
- Navigate to settings
- Review room membership summary

## 2. UI Layout Architecture
- Structural layout:
- Profile hero card at top
- Information fields grouped in stacked cards
- Optional AI learning profile block in refined prototype
...
|---|---|---|---|---|---|---|---|---|---|---|
| profile-avatar | Avatar block | Media display/upload trigger | Desktop, tablet, mobile | Show and optionally edit avatar | Open upload | default, uploading, error | Authenticated | profile.avatarUrl | File type and size | Preview before confirm |
| profile-fields | Profile detail fields | Read/write rows | Desktop, tablet, mobile | Show account data | Edit where allowed | read-only, editing, saving | Authenticated | profile entity | Field-specific | Some fields locked by SSO |
| profile-learning | AI learning profile | Insight card | Desktop, tablet, mobile | Show inferred strengths | Review | loading, loaded | Authenticated and AI enabled | ai.learningProfile | Confidence thresholds | Refined prototype-only surface |
...
## 5. State Management
- Initial state: Hydrating profile, editing, save success, save validation failure, avatar upload pending.
...
## 6. Business Logic
- Screen logic:
- Editable fields vary by tenant and SSO policy
- Institution-verified fields may be locked
- Profile completion can affect onboarding reminders
...
## 8. Edge Cases
- Avatar upload fails after optimistic preview
- SSO-managed name field cannot be changed
- User loses edit permission due to tenant policy
