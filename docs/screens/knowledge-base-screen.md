...
## 1. Screen Overview
- Screen purpose: Allow users to ask natural-language questions over their room content and receive source-grounded answers.
- Business goal: Turn accumulated classroom content into reusable institutional knowledge.
- User goal: Ask a study question, get a direct answer, and inspect the underlying room evidence.
- Primary actions:
- Ask a knowledge-base question
- Read AI answer
- Open cited sources
- Secondary actions:
- Reuse suggested prompts
- Refine question
- Cross-check against raw posts

## 2. UI Layout Architecture
- Structural layout:
- Question input near the top
- AI answer card below with label and answer text
- Source chips or links under the generated response
...
|---|---|---|---|---|---|---|---|---|---|---|
| kb-query | Knowledge question input | Input | Desktop, tablet, mobile | Capture NL question | Type, Enter | idle, focus, answering | Authenticated and AI enabled | kb.query | Non-empty, size-bounded | Should support multiline in future |
| kb-answer-card | AI answer card | Generated content panel | Desktop, tablet, mobile | Display grounded answer | Read | hidden, loading, answered, low-confidence, error | Authenticated and AI enabled | kb.answer | Must include source set when grounded | Hidden until first successful or failed ask |
| kb-sources | Source chips/links | Link set | Desktop, tablet, mobile | Open supporting evidence | Click | visible when sources exist | Authenticated with room access | kb.sources[] | Each source re-authorized on open | Critical trust affordance |
...
## 5. State Management
- Initial state: Idle, answering, answered, no-answer, low-confidence, timeout.
...
## 6. Business Logic
- Screen logic:
- Only content from rooms the user can access may be embedded in retrieval
- Answer panel stays hidden until a valid query is submitted
- Fallback answer appears when retrieval confidence is weak
...
## 8. Edge Cases
- Ambiguous query with multiple course meanings
- No indexed content yet
- Stale source pointers after content deletion
