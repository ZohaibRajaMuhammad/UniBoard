# Rooms Discovery Screen

Source prototypes: uniboard-ai.html, uniboard-complete.html

## 1. Screen Overview
- Screen purpose: Help users browse, search, and join academic rooms.
- Business goal: Increase room participation and subject network density.
- User goal: Find relevant rooms by course, subject code, cohort, or interest and join quickly.
- Primary actions:
- Search rooms
- Open room details
- Join room
- Create room
- Secondary actions:
- Inspect membership badges
- Compare room descriptions
- Return to previous screen

## 2. UI Layout Architecture
- Structural layout:
- Header block with title and supporting subtitle
- Search bar spanning primary content width
- Card grid of discoverable rooms with status chips and metadata
...
|---|---|---|---|---|---|---|---|---|---|---|
| rooms-header | Discovery header | Text block | Desktop, tablet, mobile | Orient user | Passive | default | Authenticated | Static | None | Can carry institution-specific copy |
| rooms-search | Room search input | Input | Desktop, tablet, mobile | Filter room list | Type | idle, focus, filtering | Authenticated | query | Trim whitespace | Should support subject code and natural text |
| rooms-card-grid | Room discovery cards | Grid list | Desktop, tablet, mobile | Browse joinable rooms | Open/join | loading, loaded, empty | Authenticated | discoverableRooms[] | Permission-filtered backend result | Join state changes inline |
| rooms-create | Create room CTA | Button | Desktop, tablet, mobile | Open create modal | Open modal | default, hover | Authorized creator | modal state | Policy gate | May coexist with discovery search |
...
## 5. State Management
- Initial state: Initial grid, search-filtering, joining, joined, permission-blocked, empty results.
...
## 6. Business Logic
- Screen logic:
- Joined rooms must reflect current membership state immediately
- Private rooms require invite or approval workflow
- Sorting can bias active or instructor-verified rooms
...
- GET /api/v1/rooms/{roomId}/preview
...
## 8. Edge Cases
- Concurrent join from another device
- Room reaches membership cap
- Search returns duplicates across course aliases
