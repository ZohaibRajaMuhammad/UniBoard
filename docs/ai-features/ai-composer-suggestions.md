...
## 1. AI Objective
- Problem being solved: Students often post vague, low-context, or poorly structured content.
- Business objective: Increase post clarity and downstream engagement.

## 2. Input Pipeline
- Draft text
- Selected post type
- Room context
- Policy flags like anonymity
...
## 3. Model Logic
- Lightweight prompt or local heuristic router for chip suggestions
- Optional rewrite model for accepted actions
...
## 5. Failure Handling
- If model unavailable, fall back to deterministic suggestions by post type
- Never auto-send mutated content
