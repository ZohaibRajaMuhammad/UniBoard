...
export default function ReputationPage() {
  ...
  const expertise = (profile?.data?.expertise ?? me?.expertise ?? []) as Array<{ topic: string; score: number; confidence: string; evidence?: string }>;
            ...
            <section className="spotlight-ring glass-panel page-hero">
              <div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
                ...
                <div className="rounded-[24px] border border-[var(--app-line)] bg-black/20 p-5">
                  <p className="text-xs uppercase tracking-[0.2em] text-[var(--app-text-muted)]">Global rank</p>
                  <p className="mt-2 text-3xl font-black text-white">#{me.rank}</p>
                  <p className="mt-2 text-sm text-[var(--app-text-muted)]">Next tier at {me.nextTierTarget} XP</p>
                </div>
              </div>

              <div className="mt-6 rounded-3xl border border-[var(--app-line)] bg-black/20 p-4">
                <div className="flex items-center justify-between text-sm">
                  <span className="text-white">Progress to next tier</span>
                  <span className="text-[var(--app-text-muted)]">{Math.round(me.progress)}%</span>
                </div>
                <div className="mt-3 h-3 rounded-full bg-white/5">
                  <div className="h-3 rounded-full bg-[var(--app-primary)]" style={{ width: `${Math.max(4, me.progress)}%` }} />
                </div>
              </div>
            </section>
              ...
              <div className="space-y-5">
                <div className="glass-panel rounded-[28px] p-6">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-xs uppercase tracking-[0.2em] text-[var(--app-text-muted)]">Achievements</p>
                      <h2 className="mt-2 text-2xl font-semibold text-white">Unlocked milestones</h2>
                    </div>
                  </div>
                  <div className="mt-5 grid gap-3 sm:grid-cols-2">
                    {me.achievements.map((achievement) => (
                      <div key={achievement.label} className={achievement.unlocked ? "rounded-[24px] border border-emerald-400/25 bg-emerald-500/10 p-4 text-emerald-100" : "rounded-[24px] border border-[var(--app-line)] bg-black/20 p-4 text-[var(--app-text-muted)]"}>
                ...
                <div className="glass-panel rounded-[28px] p-6">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-xs uppercase tracking-[0.2em] text-[var(--app-text-muted)]">Recent XP activity</p>
              ...
              <div className="space-y-5">
                <div className="glass-panel rounded-[28px] p-6">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-xs uppercase tracking-[0.2em] text-[var(--app-text-muted)]">Streak and expertise</p>
                      <h2 className="mt-2 text-2xl font-semibold text-white">Current momentum</h2>
                    </div>
                  </div>
                  <div className="mt-5 grid gap-4">
                    <div className="rounded-[24px] border border-[var(--app-line)] bg-black/20 p-4">
                      <p className="text-xs uppercase tracking-[0.2em] text-[var(--app-text-muted)]">Streak</p>
                      <p className="mt-2 text-3xl font-black text-white">{me.streak} days</p>
                    </div>
                    <div className="rounded-[24px] border border-[var(--app-line)] bg-black/20 p-4">
                      <p className="text-xs uppercase tracking-[0.2em] text-[var(--app-text-muted)]">Contribution mix</p>
                      <p className="mt-2 text-sm text-[var(--app-text-soft)]">{me.stats.posts} posts | {me.stats.comments} comments | {me.stats.upvotesReceived} upvotes</p>
                    </div>
                  </div>
                  {profileError ? (
                    <div className="mt-4 rounded-2xl border border-red-400/20 bg-red-500/10 p-4 text-sm text-red-100">{profileError}</div>
                  ) : profile?.data?.summary ? (
                    <p className="mt-4 text-sm leading-7 text-[var(--app-text-muted)]">{profile.data.summary}</p>
                  ) : null}
                  <div className="mt-4 space-y-3">
                    {expertise.length === 0 ? (
                      ...
                      expertise.map((item) => (
                        <div key={item.topic} className="rounded-2xl border border-[var(--app-line)] bg-black/20 p-4">
                          <div className="flex items-center justify-between gap-4">
                            <p className="font-medium text-white">{item.topic}</p>
                            <span className="text-sm text-[var(--app-text-soft)]">{item.score}</span>
                          </div>
                          <div className="mt-3 h-2 rounded-full bg-white/5">
                            <div className="h-2 rounded-full bg-[var(--app-primary)]" style={{ width: `${item.score}%` }} />
                          </div>
                          <p className="mt-2 text-xs text-[var(--app-text-muted)]">{item.confidence} confidence inference</p>
                          {item.evidence ? <p className="mt-1 text-xs text-[var(--app-text-muted)]">{item.evidence}</p> : null}
                        </div>
                      ))
                    )}
                  </div>
                ...
                <div className="glass-panel rounded-[28px] p-6">
                  <p className="text-xs uppercase tracking-[0.2em] text-[var(--app-text-muted)]">Leaderboard context</p>
                  <div className="mt-5 space-y-3">
                    {leaderboard?.slice(0, 5).map((entry: NonNullable<typeof leaderboard>[number]) => (
                      <div key={entry.userId} className="flex items-center justify-between gap-4 rounded-2xl border border-[var(--app-line)] bg-black/20 p-4">
                        <div>
                  ...
                  <Link href="/leaderboard" className="mt-5 inline-flex text-sm font-medium text-[var(--app-primary-strong)] transition hover:text-white">
