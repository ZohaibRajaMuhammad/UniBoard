...
export function RoomCard({ room }: { room: Doc<"rooms"> }) {
  return (
    <Link href={`/rooms/${room._id}`} className="lift-on-hover spotlight-ring glass-panel block rounded-[var(--radius-panel)] p-5 transition hover:border-brand-400/30 sm:p-6">
      <div className="flex items-start justify-between gap-4">
        ...
        <span className="shrink-0 rounded-full border border-white/10 bg-white/[0.04] px-3 py-1 text-xs text-gray-400">{room.batch}</span>
      </div>
