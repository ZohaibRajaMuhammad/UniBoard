...
export function PinnedPostsBanner({ posts }: { posts: Doc<"posts">[] }) {
  ...
  return (
    <div className="px-4 py-4 sm:px-6">
      <div className="alert-surface mx-auto max-w-6xl overflow-hidden px-5 py-5 transition-all duration-300 sm:px-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="max-w-3xl">
            ...
            <div className="mt-4 rounded-[24px] border border-amber-400/20 bg-white/55 px-4 py-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.35)] dark:bg-white/[0.03]">
          ...
          <div className="min-w-0 lg:w-[20rem]">
            ...
            <div className="mt-3 space-y-2">
              {remaining.length === 0 ? (
                <div className="rounded-[20px] border border-dashed border-amber-400/20 bg-white/40 px-4 py-3 text-sm text-[var(--app-text-muted)] dark:bg-white/[0.03]">
                  No other pinned notes right now.
                </div>
              ) : (
                remaining.map((post) => (
                  <div key={post._id} className="rounded-[20px] border border-amber-400/16 bg-white/50 px-4 py-3 dark:bg-white/[0.03]">
