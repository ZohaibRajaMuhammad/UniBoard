      ...
      <div className="alert-surface overflow-hidden px-5 py-5 transition-all duration-300 sm:px-6">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          <div className="max-w-3xl">
            <div className="flex items-center gap-3 text-[var(--app-warning)]">
          ...
          <div className="min-w-0 lg:w-[20rem]">
            <p className="text-[11px] font-semibold uppercase tracking-[0.22em] text-[var(--app-warning)]/80">Additional pinned signal</p>
                ...
                remaining.map((post) => (
                  <div key={post._id} className="rounded-[20px] border border-amber-400/16 bg-white/60 px-4 py-3 dark:bg-white/[0.03]">
                    <div className="flex items-start gap-2">
                      ...
                      <p className="text-sm leading-6 text-[var(--app-text-soft)]">{post.content}</p>
                    </div>
                  </div>
                ))
              )}
